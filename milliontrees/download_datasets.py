...
def main():
    ...
    parser.add_argument('--datasets', nargs='*', default=None,
                        help=f'Specify a space-separated list of dataset names to download. If left unspecified, the script will download all of the official benchmark datasets. Available choices are {milliontrees.supported_datasets}.')
    ...
    for dataset in config.datasets:
        if dataset not in milliontrees.supported_datasets:
            raise ValueError(f'{dataset} not recognized; must be one of {milliontrees.supported_datasets}.')

    print(f'Downloading the following datasets: {config.datasets}')
    for dataset in config.datasets:
        print(f'=== {dataset} ===')        
        milliontrees.get_dataset(
            dataset=dataset,
