...
class DetectionMAP(Metric):
    ...
    def __init__(self,
        ...
        if name is None:
            if iou_thresholds is not None and len(iou_thresholds) == 1:
                name = f"AP{int(round(iou_thresholds[0] * 100))}"
            else:
                name = "mAP"
        super().__init__(name=name)
...
class CountingError(ElementwiseMetric):
    ...
    def _compute_group_wise(self, y_pred, y_true, g, n_groups):
        if len(y_pred) == 0:
            group_metrics = torch.full((n_groups,), float("nan"))
            group_counts = torch.zeros(n_groups, dtype=torch.long)
            return group_metrics, group_counts, torch.tensor(float("nan"))

        elementwise = self._compute_element_wise(y_pred, y_true)
        finite_mask = ~torch.isnan(elementwise)
        group_metrics = torch.full((n_groups,), float("nan"))
        group_counts = torch.zeros(n_groups, dtype=torch.long)
        for group_idx in range(n_groups):
            mask = (g == group_idx) & finite_mask
            count = int(mask.sum().item())
            group_counts[group_idx] = count
            if count > 0:
                group_metrics[group_idx] = elementwise[mask].mean()

        evaluated = group_metrics[group_counts > 0]
        if evaluated.numel() == 0:
            worst_group_metric = torch.tensor(float("nan"))
        else:
            worst_group_metric = self.worst(evaluated)
        return group_metrics, group_counts, worst_group_metric
