...
class MillionTreesDataset:
    ...
    def _drop_incomplete_tiles(self,
                               df,
                               splits,
                               data_dir,
                               verbose=True,
                               **rule_kwargs):
        """Drop rows from ``splits`` whose tile is not annotated wall to wall.

        The packaged ``complete`` flag is per source, which is too coarse for the TLS
        validation sources: they mix interior tiles annotated edge to edge with edge tiles
        where the plot footprint clips a corner and the rest is unlabelled forest. AP charges
        every detection in the unlabelled part as a false positive, so those tiles measure the
        annotation protocol rather than the model
        (``milliontrees.common.tile_completeness``, ``notes/validation_ap_completeness.md``).

        The decision comes from ``data_prep/tile_completeness.csv``, generated by
        ``data_prep/flag_tile_completeness.py`` from a geometry that has extent. Reading it
        rather than recomputing per task is what lets the point loader evaluate the same tiles
        as the box and polygon loaders: a stem point sits half a crown inside the tile edge and
        covers no canopy pixels, so the rule cannot be applied to centroids directly. If the
        manifest has no entry for this version, the rule is computed in place instead of
        silently keeping every tile.
        """
        from milliontrees.common.tile_completeness import (
            complete_tile_names, manifest_complete_tiles)

        splits = ["validation"] if splits is True else list(splits)
        target = df[df["split"].isin(splits)]
        if target.empty:
            return df

        source = "manifest"
        complete = manifest_complete_tiles(self.version, splits)
        if complete is None:
            source = "computed"
            complete = complete_tile_names(target, data_dir, **rule_kwargs)
        complete &= set(target["filename"].unique())

        keep = ~df["split"].isin(splits) | df["filename"].isin(complete)
        n_tiles = target["filename"].nunique()
        if verbose:
            print(
                f"[MillionTrees] complete_tiles_only ({source}): kept {len(complete)} of "
                f"{n_tiles} {'/'.join(splits)} tiles "
                f"({n_tiles - len(complete)} edge tiles dropped)")
