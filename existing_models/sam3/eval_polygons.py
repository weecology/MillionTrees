"""Evaluate SAM3 on MillionTrees TreePolygons (text-prompted instance segmentation -> masks)."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="SAM3 on TreePolygons (instance masks).")
    ...
    for b_idx, (metadata, images, targets) in enumerate(test_loader):
        ...
        for res, target in zip(results, targets):
            ...
            if masks is None or len(masks) == 0:
                y_pred = {"y": torch.zeros((0, 0, 0), dtype=torch.bool),
                ...
                y_pred = {"y": masks_t, "labels": torch.zeros(masks_t.shape[0], dtype=torch.int64),
                          "scores": scores_t}
    ...
    if args.output_dir:
        ...
        with open(os.path.join(args.output_dir, f"results_polygons_{args.split_scheme}.json"), "w") as f:
            json.dump({"model": "SAM3", "task": "TreePolygons",
