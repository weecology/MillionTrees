...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="SAM3 -> Points on MillionTrees TreePoints (mask centroid per instance).")
...
def masks_to_centroids(masks: torch.Tensor) -> torch.Tensor:
    ...
    for m in masks_u8:
        ...
        cx = (xs * m).sum() / count
        cy = (ys * m).sum() / count
        centroids.append(torch.stack([cx, cy], dim=0))
    if len(centroids) == 0:
        return torch.zeros((0, 2), dtype=torch.float32)
    return torch.stack(centroids, dim=0).to(torch.float32)
    ...
    for b_idx, batch in enumerate(test_loader):
        ...
        for res, target in zip(results, targets):
            ...
            if masks is None or len(masks) == 0:
                ...
                scores_t = torch.as_tensor(scores, dtype=torch.float32).detach().to("cpu") if scores is not None else torch.zeros((centroids.shape[0],), dtype=torch.float32)
