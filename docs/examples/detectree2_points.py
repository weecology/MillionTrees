...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Detectree2 on MillionTrees TreePoints (centroid of envelopes).")
    ...
    parser.add_argument("--score-threshold",
                        type=float,
                        default=0.25,
                        help="Confidence threshold for polygons before centroiding")
...
def _predict_polygons_for_image(
    ...
    raise SystemExit(
        ...
        "in docs/examples/detectree2_points.py:_predict_polygons_for_image to match your Detectree2 version."
    ) from last_err
...
def main() -> None:
    ...
    for b_idx, batch in enumerate(test_loader):
        metadata, _images, targets = batch
        ...
        for img_path, target in zip(image_paths, targets):
            ...
            if len(gdf) == 0:
                ...
                # centroid of envelopes -> (x, y)
                centroids = np.vstack([[geom.centroid.x, geom.centroid.y] for geom in envelopes]).astype(np.float32)
