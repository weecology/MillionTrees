...
'existing_split': Optional pre-assigned split. Supported values are ``train``, ``test``, and ``validation``. When set, packaging honors the value across all split schemes (``within-distribution``, ``out-of-distribution``, ``crossgeometry``). Use ``validation`` for held-out TLS or other independent ground-truth sources that must never enter training or the standard test splits — for example, post-hoc zero-shot evaluation after models are trained and evaluated on ``train``/``test``.
